...
class external extends external_api {
    ...
    public static function get_course_world_group_ladder($courseid, $page, $perpage) {
        ...
        if (!$config->get('enablegroupladder') || $adminconfig->get('context') == CONTEXT_SYSTEM) {
            throw new moodle_exception('nopermissions', '', '', 'view_group_ladder_page');
        }

        // Leaderboard.
        $leaderboard = new \local_xp\local\leaderboard\course_group_leaderboard(di::get('db'), $courseid);
    ...
    public static function get_course_world_group_ladder_returns() {
        return new external_single_structure([
            ...
            'ranking' => new external_multiple_structure(
                new external_single_structure([
                    'rank' => new external_value(PARAM_INT),
                    'state' => self::state_description(false, true)
                ])
            ),
        ]);
    }
    ...
    public static function get_setup($courseid) {
        ...
        return [
            ...
            'config' => [
                ...
                'enablegroupladder' => $config->get('enablegroupladder') && $adminconfig->get('context') == CONTEXT_COURSE,
            ],
            'visuals' => [
    ...
    public static function get_setup_returns() {
        return new external_single_structure([
            ...
            'config' => new external_single_structure([
                ...
                'enablegroupladder' => new external_value(PARAM_BOOL),
            ], '', VALUE_OPTIONAL),
    ...
    public static function serialize_state(state $state, $withuser = false, $anonymously = false) {
        ...
        if ($withuser && $state instanceof user_state) {
            ...
            $data['group'] = [
                'id' => $state->get_id(),
                'name' => format_string($group->name, true, ['context' => context_course::instance($courseid)]),
                'imageurl' => static::serialize_url(get_group_picture_url($group, $courseid, false))
            ];
        }
     ...
     */
    public static function state_description($withuser = false, $withgroup = false) {
        $params = [
            'id' => new external_value(PARAM_INT),
