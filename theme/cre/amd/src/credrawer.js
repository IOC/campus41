...
define(['jquery', 'core/custom_interaction_events', 'core/log'],
    ...
    var SELECTORS = {
        TOGGLE_REGION: '[data-region="credrawer-toggle"]',
        TOGGLE_ACTION: '[data-action="toggle-credrawer"]',
        TOGGLE_TARGET: 'aria-controls',
        TOGGLE_SIDE: 'left',
        BODY: 'body',
        SECTION: '.list-group-item[href*="#section-"]',
        NAV_DRAWER: '#nav-credrawer',
        BUTTON_CLOSE: '.button-close',
        ITEM_ACTIVE: '.list-group-item.active'
    };
    ...
    var Drawer = function() {
        ...
        var active = $(SELECTORS.ITEM_ACTIVE);
        var position = 0;

        if (active.length) {
            position = active.offset().top;
            $(SELECTORS.NAV_DRAWER).animate({scrollTop: position - 137}, 500);
        }
    };
    ...
    Drawer.prototype.toggleDrawer = function(e) {
        var trigger = $(e.target).closest(SELECTORS.TOGGLE_ACTION);
        var drawerid = trigger.attr('aria-controls');
        var drawer = $(document.getElementById(drawerid));
        var body = $(SELECTORS.BODY);
        var side = trigger.attr('data-side');
    ...
    Drawer.prototype.registerEventListeners = function() {
        ...
        $(SELECTORS.BUTTON_CLOSE).click(function() {
            this.closeAll();
        }.bind(this));
    };
