...
class activity_navigation extends \core_course\output\activity_navigation {
     ...
     */
    public function __construct($prevmod, $nextmod, $activitylist = array(), $cremodule = false, $creindexpage = false) {
        global $OUTPUT;

        $this->cremodule    = $cremodule;
        $this->creindexpage = $creindexpage;
        ...
        if ($prevmod) {
            ...
            if (!$cremodule) {
                ...
                $button = html_writer::tag('div', get_string('previous', 'moodle'), array('class' => 'btn-text'));
                $icon = new pix_icon('images/button-prev', $linkname, 'theme');
                $this->prevlink = new \action_link($linkurl, $button, null, $attributes, $icon);
            }
        }
        ...
        if ($nextmod) {
            ...
            if (!$cremodule) {
                ...
                $customtext = $this->creindexpage ? get_string('lestgo', 'theme_cre') : get_string('next', 'moodle');
                $button = html_writer::tag('div', $customtext, array('class' => 'btn-text'));
                $customicon = $this->creindexpage ? 'images/somhi-boto-bck' : 'images/button-next';
                $icon = new pix_icon($customicon, $linkname, 'theme');
                $this->nextlink = new \action_link($linkurl, $button, null, $attributes, $icon);
            }
        }
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        $data->cremodule    = $this->cremodule;
        $data->creindexpage = $this->creindexpage;
